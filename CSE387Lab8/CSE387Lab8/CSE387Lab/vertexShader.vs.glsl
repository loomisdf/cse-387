	...
	switch (object.textureMode) {
		...
		case 0:
			...
			for (int i = 0; i < MaxLights; i++)  {
				...
				color0 += vec4( shadingCaculation( lights[i], object ), 1.0);
			}
		...
		case 1:
			...
			color0 = texture2D( diffuseSampler, TexCoord0.st);
			break;
		...
		case 2:
			...
			vec4 textColor =  texture2D( diffuseSampler, TexCoord0.st);
			illuminatedObject.ambientMat = textColor;
			illuminatedObject.diffuseMat = textColor;

			color0 = illuminatedObject.emmissiveMat;

			for (int i = 0; i < MaxLights; i++)  {
	
				color0 += vec4( shadingCaculation( lights[i], illuminatedObject ), 1.0);
			}
