...
void rotation(int value)
{
	...
	case(1) :
		// Render polygon fronts and fill them
		scene.setRotation(glm::vec3(0.0f, 1.0f, 0.0f));
		break;
	case(2) :
		// Render polygon fronts in wire frame
		scene.setRotation(glm::vec3(1.0f, 0.0f, 0.0f));
		break;
	default:
		cout << "Unknown rotation Mode!" << endl;
	}
	// Signal GLUT to call display callback
	glutPostRedisplay();
}
...
int main(int argc, char** argv)
{
	...
	GLuint world_Window = glutCreateWindow("CSE 387 Lab 4");
	...
	GLuint menu9id = glutCreateMenu(rotation);
	glutAddMenuEntry("[0,1,0]", 1);
	glutAddMenuEntry("[1,0,0]", 2);
	...
	glutAddSubMenu("Rotation", menu9id);
