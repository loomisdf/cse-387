...
GLint TextureFromFile(const char* path, const string& directory) 
{
	...
	if (imagen == NULL || width == 0 || height == 0) {
		std::cerr << "ERROR: Unable to load " << filename << "!" << endl;
		return 0;
	}
	...
	cout << filename.c_str() << ": width " << width << " height " << height << endl;
...
void AssimpModel::initialize()
{
	...
	material.setTextureMapped(REPLACE_AMBIENT_DIFFUSE);
...
void AssimpModel::loadModel(const string& path) {
	...
	if (!scene || scene->mFlags == AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) {
		std::cerr << "ERROR::ASSIMP::" << "Could not load " << path << importer.GetErrorString() << std::endl;
		return;
	}
	else {
...
{
	cout << "AssimpModel::update: " << this << endl;
