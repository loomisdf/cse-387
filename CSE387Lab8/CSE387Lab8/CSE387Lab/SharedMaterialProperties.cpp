...
void SharedMaterialProperties::setShaderMaterialProperties(GLuint shaderProgram, Material material)
{
	...
	gSamplerLocation = glGetUniformLocation(shaderProgram, "diffuseSampler");
	// Set it to 0
	...
	if (material.textureMode != NO_TEXTURE) {
		...
		//glBindTexture(GL_TEXTURE_2D, material.textureObject);
		//glBindTextureUnit(gSamplerLocation, material.textureObject);
	}
	else {
		//glBindTextureUnit(gSamplerLocation, 0);
