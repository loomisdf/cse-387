...
static void polygonMode(int value)
{
	switch (value) {
	case(1) :
...
static void pointSize(int value)
{
...
static void lineWidthMenu(int value)
{
	glLineWidth((GLfloat)value);
	cout << "Line width is " << value << " pixels." << endl;
}

static void antiAliasMenu(int value)
{
	switch (value) {
	case(1) :
...
static void frontFaceMenu(int value)
{
	switch (value) {
	case(1) :
		glFrontFace(GL_CCW);
...
static void cullFaceMenu(int value)
{
	switch (value) {
	case(1) :
...
static void faceCulling(int value)
{
	switch (value) {
	case(1) :
...
static void depthTesting(int value)
{
	switch (value) {
	case(1) :
...
static void shader(int value)
{
	scene.selectShader(value);
}
...
static void createMenus()
{
	...
	GLuint menu9id = glutCreateMenu(shader);
	glutAddMenuEntry("PerPixel", 0);
	glutAddMenuEntry("PerVertex", 1);

	GLuint menu10id = glutCreateMenu(mode);
...
int main(int argc, char** argv)
{
	...
	GLuint world_Window = glutCreateWindow("CSE 387 Lab 9");
