...
struct Material
{
	...
	{
		ambientMat = glm::vec4( 0.5f, 0.5f, 0.0f, 1.0f );
		diffuseMat = glm::vec4( 0.5f, 0.5f, 0.0f, 1.0f );
	...
	void setTextureMapped(TextureMode textureMode)
	{
		...
		glBufferSubData(GL_UNIFORM_BUFFER, textureMappedLoc, sizeof(GLint), &textureM);
	}
